...
func (handle *AccountHandler) CreateAccount(c echo.Context) error {
	...
	_, err = handle.store.GetAccount(u.Email)
	...
	err = checkFormValue(nameRegex, u.Fname)
	if err != nil {
		// Future: Change this to show server error and on dev show server error
		re := fillResponse("Invalid", "Invalid characters in first name")
		return c.JSON(http.StatusUnprocessableEntity, re)
	}
	err = checkFormValue(nameRegex, u.Lname)
	if err != nil {
	...
	// Make sure to check for special characeters
	//https://stackoverflow.com/questions/19605150/regex-for-password-must-contain-at-least-eight-characters-at-least-one-number-a
	if utf8.RuneCountInString(u.Password) < 8 {
		re := fillResponse("Invalid", "Invalid number of characters in password")
		return c.JSON(http.StatusUnprocessableEntity, re)
	}
	...
	err = handle.store.CreateAccount(u.Fname, u.Lname, u.Email, u.Username, string(hash))
	if err != nil {
		fmt.Println(err)
		re := fillResponse("Failed", "Server error, try again later")
		return c.JSON(http.StatusInternalServerError, re)
...
func (handle *AccountHandler) Login(c echo.Context) error {
	...
	sessionid := uuid.New()
	cookie := createCookie(sessionid.String(), u.Remember)
	c.SetCookie(cookie)
